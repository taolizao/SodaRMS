...
struct logger {
    ...
    int  slow_log_fd; /* slow log file descriptor */
    int  slow_log_threshold; /* over this threshold should be logged in slow log */
...
#define SLOW_QUERY_TIME_LIMIT 30
...
int log_slow(struct msg *msg, uint64_t msg_dealing_time, \
              struct context *ctx, struct server *server);
int slow_log_hexdump(char *buf, struct msg *pmsg);
int log_init(int level, char *filename, struct instance *nci);
