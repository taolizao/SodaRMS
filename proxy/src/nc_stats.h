...
#define STATS_POOL_CODEC(ACTION)                                                                            \
    ...
    ACTION( total_qps,              STATS_GAUGE,        "# total (both read and write)qps per server_pool") \
    ACTION( read_qps,               STATS_GAUGE,        "# total read qps per server_pool")                 \
    ACTION( write_qps,              STATS_GAUGE,        "# total write qps per server_pool")                \
    ACTION( real_total_qps,         STATS_GAUGE,        "# real total qps per server_pool")                 \
    ACTION( real_read_qps,          STATS_GAUGE,        "# real total read qps per server_pool")            \
    ACTION( real_write_qps,         STATS_GAUGE,        "# real total write qps per server_pool")           \
    ACTION( consuming_token,        STATS_GAUGE,        "# consuming token per server_pool")                \
    /* pool behavior */                                                                                     \
...
#define STATS_SERVER_CODEC(ACTION)                                                                          \
    ...
    ACTION( slow_cmd_count,         STATS_COUNTER,      "total amount of slow command")                     \
...
struct stats {
    ...
    struct string       proxy_total_qps_str;  /* proxy total QPS string */
    struct string       proxy_read_qps_str;  /* proxy read QPS string */
    struct string       proxy_write_qps_str;  /* proxy write QPS string */

    volatile int        aggregate;      /* shadow (b) aggregate? */
    volatile int        updated;        /* current (a) updated? */

    struct timeval      time_begin;     /* the start time of QPS calculation */
    struct timeval      time_end;       /* the end time of QPS calculation*/
    int                 time_tag;       /* execute "gettimeofday" or not */

    struct context      *ptr_to_context;/* point to the context */
};
...
struct stats *stats_create(uint16_t stats_port, char *stats_ip, int stats_interval, char *source, struct context *ctx);
