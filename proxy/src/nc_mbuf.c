...
static uint32_t nall_mbuf; /* # mbuf */ 
static size_t mbuf_memuse_limit; /* # mbuf can alloc */ 
static uint32_t mbuf_freelist_limit; /* # mbuf can cache */ 
static uint32_t mbuf_soft_limit;
...
_mbuf_get(void)
{
    ...
    if(nall_mbuf >= mbuf_memuse_limit) {
        errno = ENOMEM;
        log_warn("cannot allocate memory[OOM], max-memory-limit: %lu", 
                mbuf_memuse_limit*mbuf_chunk_size);
        return NULL;
    }
    ...
    nall_mbuf++;
    if(nall_mbuf >= mbuf_soft_limit) {
        mem_exceed_soft_limit = 1;
    }
...
mbuf_put(struct mbuf *mbuf)
{
    ...
    if(nfree_mbufq >= mbuf_freelist_limit) {
        mbuf_free(mbuf);
        return;
    }
    nfree_mbufq++;
...
mbuf_init(struct instance *nci)
{
    ...
    mbuf_memuse_limit = (nci->maxmemory << 20) / mbuf_chunk_size;
    mbuf_soft_limit = (uint32_t)((double)mbuf_memuse_limit * (double)MBUF_MEM_SOFT_LIMIT);
    mbuf_freelist_limit = (1 << 28) / mbuf_chunk_size; /*256MB*/ 
 ...
 */
rstatus_t
mbuf_read_string(struct mbuf *mbuf, const char c, struct string *read_string)
{
    rstatus_t status;
    uint32_t read_size;
    uint8_t *p;
