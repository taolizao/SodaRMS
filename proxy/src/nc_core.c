...
core_ctx_create(struct instance *nci)
{
    rstatus_t status;
    struct context *ctx;
    struct conn *sentinel_connection;
    ...
    ctx->zero_warn_msg_interval = nci->zero_warn_msg_interval;
    ctx->zero_slow_cmd_count_interval = nci->zero_slow_cmd_count_interval;
    ...
    sentinel_connection = sentinel_connect(ctx);
    if (sentinel_connection == NULL) {
        server_pool_disconnect(ctx);
        event_deinit(ctx);
        stats_destroy(ctx->stats);
        sentinel_deinit(ctx->sentinel);
    ...
    if (status != NC_OK) {
        sentinel_connection->close(ctx, sentinel_connection);
        server_pool_disconnect(ctx);
        event_deinit(ctx);
        stats_destroy(ctx->stats);
        sentinel_deinit(ctx->sentinel);
...
core_close(struct context *ctx, struct conn *conn)
{
    ...
    if (conn->client) {
        type = 'c';
        addrstr = conn->peer_ip;
    } else {
        type = conn->proxy ? 'p' : 's';
        addrstr = nc_unresolve_addr(conn->addr, conn->addrlen);
    }
    if(conn->pipeline_max_num > CONN_PIPELINE_LIMIT1) {
        log_level = LOG_WARN;
    }
    
    log_debug(log_level, "close %c %d '%s' on event %04"PRIX32" eof %d done "
              "%d pipeline %d rb %zu sb %zu%c %s", type, conn->sd, addrstr, conn->events,
              conn->eof, conn->done, conn->pipeline_max_num, conn->recv_bytes, conn->send_bytes,
