...
stats_create_buf(struct stats *st)
{
    ...
    size += st->proxy_total_qps_str.len;
    size += int64_max_digits;
    size += key_value_extra;

    size += st->proxy_read_qps_str.len;
    size += int64_max_digits;
    size += key_value_extra;

    size += st->proxy_write_qps_str.len;
    size += int64_max_digits;
    size += key_value_extra;
...
stats_add_string(struct stats *st, struct string *key, struct string *val)
{
    ...
    n = nc_snprintf(pos, room, "\"%.*s\":\"%.*s\", ", key->len, key->data,
                    val->len, val->data);
    if (n < 0 || n >= (int)room) {
        log_debug(LOG_WARN, "stats buffer is not large enough");
        return NC_ERROR;
    }
...
stats_add_header(struct stats *st)
{
    rstatus_t status;
    struct stats_buffer *buf;
    int64_t cur_ts, uptime, proxy_qps = 0, proxy_read_qps = 0, proxy_write_qps = 0;
    uint32_t i, k;

    struct string qps_field, read_qps_field, write_qps_field;
    string_set_text(&qps_field, "real_total_qps");
    string_set_text(&read_qps_field, "real_read_qps");
    string_set_text(&write_qps_field, "real_write_qps");
    ...
    for (i = 0; i <array_n(&st->sum); i++) {
       struct stats_pool *stp = array_get(&st->sum, i); 
        ...
        for (k = 0;k < array_n(&stp->metric); k++) {
            struct stats_metric *stm = array_get(&stp->metric, k);

            if (!string_compare(&stm->name, &qps_field)) {
                proxy_qps += stm->value.counter;
            } else if (!string_compare(&stm->name, &read_qps_field)) {
                proxy_read_qps += stm->value.counter;
            } else if (!string_compare(&stm->name, &write_qps_field)) {
                proxy_write_qps += stm->value.counter;
            }
        }
    }
    ...
    status = stats_add_num(st, &st->proxy_total_qps_str, proxy_qps);
    if (status != NC_OK) {
        return status;
    }

    status = stats_add_num(st, &st->proxy_read_qps_str, proxy_read_qps);
    if (status != NC_OK) {
        return status;
    }

    status = stats_add_num(st, &st->proxy_write_qps_str, proxy_write_qps);
    if (status != NC_OK) {
        return status;
    }
...
stats_copy_metric(struct stats *st, struct array *metric)
{
    ...
    string_set_text(&qps_field, "total_qps");
    string_set_text(&read_qps_field, "read_qps");
    string_set_text(&write_qps_field, "write_qps");

    for (i = 0; i < array_n(metric); i++) {
        struct stats_metric *stm = array_get(metric, i);

        /* when returning the result of 
         * json,just ignore these field,*/
        if (!string_compare(&stm->name, &qps_field) 
            || !string_compare(&stm->name, &read_qps_field) 
            || !string_compare(&stm->name, &write_qps_field) ) {
            continue;
        }
...
stats_send_rsp(struct stats *st)
{
    ...
    if (status != NC_OK) {
        log_debug(LOG_WARN, "failed to produce a response");
        close(sd);
        return status;
    }
...
static void
stats_qps_calculation(struct stats *st, double tick)
{
    uint32_t i, k;
    struct context *ctx = st->ptr_to_context;
    struct server_pool *pool;
    struct string qps_field, read_qps_field, write_qps_field;
    struct string real_qps_field, real_read_qps_field, real_write_qps_field;
    struct string consuming_token;
    double quota_prewarning_ratio;

    string_set_text(&qps_field, "total_qps");
    string_set_text(&read_qps_field, "read_qps");
    string_set_text(&write_qps_field, "write_qps");
    string_set_text(&real_qps_field, "real_total_qps");
    string_set_text(&real_read_qps_field, "real_read_qps");
    string_set_text(&real_write_qps_field, "real_write_qps");
    string_set_text(&consuming_token, "consuming_token");

    st->time_tag = 1;
    gettimeofday(&st->time_end, NULL);
    tick = 1000000 * (st->time_end.tv_sec - st->time_begin.tv_sec) + (st->time_end.tv_usec - st->time_begin.tv_usec); 
    tick /= 1000000;
    
    /* calculate the QPS */
    for (i = 0; i <array_n(&st->sum) ; i++) {
        struct stats_pool *stp = array_get(&st->sum, i); 
        int64_t tmp_total_qps = 0, tmp_read_qps = 0, tmp_write_qps = 0;
        ...
        for (k = 0; k < array_n(&stp->metric); k++) {
            ...
            if (!string_compare(&stm->name, &qps_field)) {
                tmp_total_qps = stm->value.counter / tick;
                stm->value.counter = 0;
            } else if (!string_compare(&stm->name, &read_qps_field)) {
                tmp_read_qps = stm->value.counter / tick;
                stm->value.counter = 0;
            } else if (!string_compare(&stm->name, &write_qps_field)) {
                tmp_write_qps = stm->value.counter / tick;
                stm->value.counter = 0;
            }
            
            if (!string_compare(&stm->name, &real_qps_field)) {
                stm->value.counter = tmp_total_qps;
            } else if (!string_compare(&stm->name, &real_read_qps_field)) {
                stm->value.counter = tmp_read_qps;
            } else if (!string_compare(&stm->name, &real_write_qps_field)) {
                stm->value.counter = tmp_write_qps;
            }

            /* quota pre-warning,if the ratio of used token 
             * is more than 80 percent,then log it */
            if (!string_compare(&stm->name, &consuming_token)) {
                quota_prewarning_ratio = (double)stm->value.counter / (double)pool->quota;
                
                if (quota_prewarning_ratio > 0.8f) {
                    log_warn("The quota of server_pool'%.*s' has been used more than 80 percent,consuming token is:%d, token used ratio is %0.2f%%, quota is %d",
                              pool->name.len, pool->name.data, stm->value.counter, quota_prewarning_ratio*100, pool->quota);
                }
            }
        }
    }
...
stats_loop(void *arg)
{
    ...
    gettimeofday(&st->time_begin, NULL);
    st->time_tag = 0;
    for (;;) {
        if (st->time_tag == 1) {
            gettimeofday(&st->time_begin, NULL);
...
stats_create(uint16_t stats_port, char *stats_ip, int stats_interval,
             char *source, struct context *ctx)
{
    ...
    string_set_text(&st->proxy_total_qps_str, "proxy_qps");
    string_set_text(&st->proxy_read_qps_str, "proxy_read_qps");
    string_set_text(&st->proxy_write_qps_str, "proxy_write_qps");
                 ...
                 stats_pool_field_t fidx)
{
    ...
    if (fidx == STATS_POOL_client_connections) {
        pool->client_connections_cnt++;
    }
                       ...
                       stats_server_field_t fidx)
{
    ...
    /* if server is a slave, stats to its master */
    if (server->master == NULL) {
        sidx = server->idx;
    } else {
        sidx = server->master->idx;
    }
                   ...
                   stats_server_field_t fidx)
{
    ...
    /* sentinel server don't have owner server pool, skip it */
