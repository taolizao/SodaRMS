 ...
 *            //
 */
#define WARNING_MESSAGE_UPPER_LIMIT     1000
#define MAX_FORBIDDEN                   WARNING_MESSAGE_UPPER_LIMIT
#define MAX_PERM_WARNING                WARNING_MESSAGE_UPPER_LIMIT
#define MAX_CONNECTION_WARNING          WARNING_MESSAGE_UPPER_LIMIT
#define MAX_QUOTA_LIMITAION_WARNING     WARNING_MESSAGE_UPPER_LIMIT

#define NC_PNAME_MAXLEN                 32

#define SERVER_RECONNECT_INTERVAL       (10 * 1000)   /* in msec */
#define SERVER_ZERO_WARN_MSG_INTERVAL   (10 * 1000)
#define SERVER_ZERO_SLOW_CMD_COUNT_INTERVAL (10 * 1000) /* in msec */
...
struct server_pool {
    ...
    int                forbidden_counter;        /* forbidden msg counter (check the MAX_FORBIDDEN above) */
    int                perm_warning;             /* perm warning msg counter (check the MAX_PERM_WARNING) */
    int                connection_is_full;       /* "connection has reached the limitaion" msg counter */
    int                quota_is_insufficient;    /* "quota has been used up" msg counter*/
    int                quota;                    /* quota assignment */ 
    int                token;                    /* token bucket */
    int                flow_control;             /* the switch of flow_control  */
    int                client_need_auth;
    int                server_need_auth;
    uint32_t                client_connections_cnt;   /* the connection of each server_pool */
};
...
struct conn *server_pool_conn(struct context *ctx, struct server_pool *pool, uint8_t *key, 
...
rstatus_t server_add_auth(struct context *ctx, struct conn *c_conn, struct conn *s_conn);
