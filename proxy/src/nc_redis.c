...
const uint32_t REDIS_RSP_STATUS_MAXLEN = 256;
...
/*
 * Return true, if the redis command is only need to forward, otherwise
 * return false
 */
bool
...
/*
 * Return true, if the redis command is either EVAL. These commands
...
redis_parse_req(struct msg *r)
{
    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_REQ_TYPE:
            ...
            msg_dump_data(r, r->token, r->rlen);
            ...
            case 9:
                ...
                if (str9icmp(m, 'b', 'r', 'o', 'a', 'd', 'c', 'a', 's', 't')) {
                    r->type = MSG_REQ_REDIS_BROADCAST;
                    break;
                }
        ...
        case SW_KEY_LF:
            ...
            case LF:
                if (redis_arg0(r)) {
                    ...
                    state = SW_ARGN_LEN;
                } else if(redis_args(r)) {
                    //let the pos reach the end to ignore the remain of the msg
                    p = b->last - 1;
                    goto done;
                } else {
                    goto error;
    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_MULTIBULK_NARG_LF:
            ...
            case LF:
                if (r->rnarg == 0) {
                    /* response is '*0\r\n' */
                    if(redis_update_and_check_done(r)) {
                        goto done;
                    }
                }
        ...
        case SW_MULTIBULK_ARGN_LEN:
            if (r->token == NULL) {
                 ...
                 *
                 */
                if (ch == '*') {    /* for sscan/hscan/zscan and geo command only */
                    p = p - 1;      /* go back by 1 byte */
...
static void
redis_filterout_empty_mbuf(struct msg *src)
{
...
static rstatus_t
redis_copy_cont_with_len(struct msg *dst, 
                         struct msg *src, 
                         struct mbuf *mbuf,
                         uint32_t len)
{
...
uint32_t redis_ana_bulklen(struct mbuf *mbuf)
{
    uint8_t *p = NULL;
    uint32_t len = 0;
    //uint32_t bytes = 0;
...
static rstatus_t
redis_copy_req_bulk(struct msg *dst, struct msg *src, uint32_t *pitemnr)
{
    struct mbuf *mbuf = NULL;
    rstatus_t status = NC_OK;
    uint32_t len = 0;

    redis_filterout_empty_mbuf(src);
    ...
    len = redis_ana_bulklen(mbuf);

    status = redis_copy_cont_with_len(dst, src, mbuf, len);
    if (status != NC_OK) {
        log_debug(LOG_VVERB, "redis_copy_req_bulk failed");
        return status;
    }
...
static rstatus_t
redis_copy_rsp_bulk(struct msg *dst, struct msg *src, uint32_t *pitemnr)
{
    struct mbuf *mbuf = NULL;
    rstatus_t status = NC_OK;
    ...
    status = redis_copy_cont_with_len(dst, src, mbuf, src->mlen);
    if (status != NC_OK) {
        log_debug(LOG_VVERB, "redis_copy_rsp_bulk failed");
        return status;
    }
...
/*
 * copy status from src to dst
 *
 * if dst == NULL, we just eat the bulk
 *
 * */
static rstatus_t
redis_copy_status(struct msg *dst, struct msg *src, uint32_t *pitemnr)
{
    struct mbuf *mbuf = NULL;
    uint8_t *p = NULL;
    uint32_t len = 0;
    rstatus_t status = NC_OK;
    char tmp[REDIS_RSP_STATUS_MAXLEN];
    ...
    /* just get status information, maxmum REDIS_RSP_STATUS_MAXLEN */
    p = mbuf->pos;
    len = 0;
    while (1) {
        if (p == mbuf->end) {
        ...
        if (len >= REDIS_RSP_STATUS_MAXLEN) {
            break;
        }
    }

    memcpy(tmp, mbuf->pos, len);
    tmp[len] = '\0';
    msg_prepend_format(dst, "$%u\r\n%s\r\n", len, tmp);
    if (status != NC_OK) {
        log_debug(LOG_VVERB, "redis_copy_status failed");
        return status;
    }
...
/*
 * copy integer from src to dst
 *
 * if dst == NULL, we just eat the bulk
 *
 * */
static rstatus_t
redis_copy_integer(struct msg *dst, struct msg *src, uint32_t *pitemnr)
{
    struct mbuf *mbuf = NULL;
    rstatus_t status = NC_OK;
    uint8_t tmp[128];
    ...
    tmplen = snprintf(tmp, sizeof(tmp), ":%u", src->integer);
    tmplen = snprintf(tmp, sizeof(tmp), "$%u\r\n:%u\r\n", tmplen, src->integer);
    status = msg_append(dst, tmp, tmplen);
    if (status != NC_OK) {
        log_debug(LOG_VVERB, "redis_copy_integer failed");
        return status;
    }
...
/*
 * copy error from src to dst
 *
 * if dst == NULL, we just eat the bulk
 *
 * */
static rstatus_t
redis_copy_error(struct msg *dst, struct msg *src, uint32_t *pitemnr)
{
    struct mbuf *mbuf = NULL;
    uint8_t *p = NULL;
    uint32_t len = 0;
    ...
    msg_prepend_format(dst, "$%u\r\n%s\r\n", len, tmp);
    if (status != NC_OK) {
        log_debug(LOG_VVERB, "redis_copy_error failed");
        return status;
    }
...
/*
 * copy one multibulk from src to dst
 *
 * if dst == NULL, we just eat the bulk
 *
 * */
static rstatus_t
redis_copy_multibulk(struct msg *dst, struct msg *src, uint32_t *pitemnr)
{
    struct mbuf *mbuf = NULL;
    rstatus_t status = NC_OK;
    ...
    status = redis_copy_cont_with_len(dst, src, mbuf, src->mlen);
    if (status != NC_OK) {
        log_debug(LOG_VVERB, "redis_copy_multibulk failed");
        return status;
    }
...
static rstatus_t
redis_compose_rsp(struct msg *response, struct msg *sub_msg, uint32_t *pitemnr)
{
    rstatus_t status = NC_OK;
    msg_type_t msgtype = sub_msg->type;

    if (msgtype == MSG_RSP_REDIS_BULK) {
        status = redis_copy_rsp_bulk(response, sub_msg, pitemnr);
    } else if (msgtype == MSG_RSP_REDIS_MULTIBULK) {
        status = redis_copy_multibulk(response, sub_msg, pitemnr);
    } else if (msgtype == MSG_RSP_REDIS_INTEGER) {
        status = redis_copy_integer(response, sub_msg, pitemnr);
    } else if (msgtype == MSG_RSP_REDIS_ERROR) {
        status = redis_copy_error(response, sub_msg, pitemnr);
    } else if (msgtype == MSG_RSP_REDIS_STATUS) {
        status = redis_copy_status(response, sub_msg, pitemnr);
    } else {
        status = NC_ERROR;
    }

    if (status != NC_OK) {
         log_debug(LOG_VVERB, "redis_compose_rsp copy failed");
    }
...
redis_pre_coalesce(struct msg *r)
{
    ...
    case MSG_RSP_REDIS_INTEGER:
        ...
        ASSERT(pr->type == MSG_REQ_REDIS_DEL  || pr->type == MSG_REQ_REDIS_BROADCAST);
    ...
    case MSG_RSP_REDIS_MULTIBULK:
        ...
        ASSERT(pr->type == MSG_REQ_REDIS_MGET || pr->type == MSG_REQ_REDIS_BROADCAST);
    ...
    case MSG_RSP_REDIS_STATUS:        
        if (pr->type == MSG_REQ_REDIS_MSET 
            || pr->type == MSG_REQ_REDIS_BROADCAST) {       /* MSET segments */
            mbuf = STAILQ_FIRST(&r->mhdr);
            r->mlen -= mbuf_length(mbuf);
            mbuf_rewind(mbuf);
        }
    ...
    default:
        if (pr->type == MSG_REQ_REDIS_BROADCAST) {
            break;
        }
                    ...
                    uint32_t key_step)
{
    ...
    for (i = 0; i < array_n(r->keys); i++) {        /* for each key */
        ...
        if (key_step == 1) {                            /* mget,del */
            ...
            status = redis_copy_req_bulk(NULL, r, &itemnr); /* eat key */
            if (status != NC_OK) {
                nc_free(sub_msgs);
                return status;
            }

            status = redis_copy_req_bulk(sub_msg, r, &itemnr);
            if (status != NC_OK) {
                nc_free(sub_msgs);
                return status;
            }
...
static rstatus_t
redis_generate_broadcastcmd(struct msg *r, struct msg **ppsub_msg)
{
    rstatus_t status = NC_OK;
    uint32_t i = 0;    
    struct keypos *kpos = NULL;
    struct msg *sub_msg = NULL;
    
    for (i = 0; i < array_n(r->keys); i++) {        /* for each key */
        kpos = array_get(r->keys, i);
        if (!sub_msg) {
...
static rstatus_t redis_fragment_broadcast_filtercmds(struct msg *r)
{
    int i = 0;
    int keylen = 0;
    uint32_t keynr = 0;
    struct keypos *kpos = NULL;
    struct instance *pnci = get_nci();  
    char buf[128];

    keynr = array_n(r->keys);
    if (keynr <= 0) {
        log_debug(LOG_ERR, "broadcast has no detail command");
        return NC_ERROR;
    }

    if (pnci->broadcastcmds.bcmdallallow) {
        return NC_OK;
    }

    kpos = array_get(r->keys, 0);
    keylen = kpos->end - kpos->start;

    for (i = 0; i < pnci->broadcastcmds.bcmdnr; i++) {
        if (!memcmp(pnci->broadcastcmds.bcmdset[i], kpos->start, keylen)) {
            return NC_OK;
        }
    }

    snprintf(buf, sizeof(buf) > keylen ? (keylen + 1) : sizeof(buf), "%s", kpos->start);
    log_debug(LOG_ERR, "broadcast command(%s) not allowed", buf);
    return NC_ERROR;
}

static rstatus_t
redis_fragment_broadcastcmd(struct msg *r, 
                            uint32_t ncontinuum, 
                            struct msg_tqh *frag_msgq)
{
    rstatus_t status = NC_OK;
    struct mbuf *mbuf = NULL;
    struct msg **sub_msgs = NULL;
    uint32_t i = 0;

    if (array_n(r->keys) <= 0 || (!ncontinuum)) {
        return NC_OK;
    }
    
    ASSERT(MSG_REQ_REDIS_BROADCAST == r->type);

    status = redis_fragment_broadcast_filtercmds(r);
    if (status != NC_OK) {
        return NC_ERROR;
    }
    ...
    ASSERT(r->frag_seq == NULL);
    r->frag_seq = nc_alloc(ncontinuum * sizeof(*r->frag_seq));
    if (r->frag_seq == NULL) {
        nc_free(sub_msgs);
        return NC_ENOMEM;
    }
    ...
    for(i = 0; i < ncontinuum; i++) {
        status = redis_generate_broadcastcmd(r, &sub_msgs[i]);
        if (status != NC_OK) {
            nc_free(sub_msgs);
            return status;
        }
    ...
    for (i = 0; i < ncontinuum; i++) {
        r->frag_seq[i] = sub_msgs[i];
        TAILQ_INSERT_TAIL(frag_msgq, sub_msgs[i], m_tqe);       
    }
    r->nfrag = ncontinuum;
...
redis_fragment(struct msg *r, uint32_t ncontinuum, struct msg_tqh *frag_msgq)
{
    ...
    case MSG_REQ_REDIS_BROADCAST:
        return redis_fragment_broadcastcmd(r, ncontinuum, frag_msgq);
    default:
        return NC_OK;
    }
}
...
void
redis_post_coalesce_broadcast(struct msg *request)
{
    struct msg *response = request->peer;
    struct msg *sub_msg = NULL;
    rstatus_t status = NC_OK;
    ...
    if ((request->narg < 2) || (!array_n(request->keys))) {
    ...
    for (i = 0; i < request->nfrag; i++) {      /* for each key */
        ...
        status = redis_compose_rsp(response, sub_msg, &itemnr);
        if (status != NC_OK) {
            response->owner->err = 1;
            return;
        }
    ...
    status = msg_prepend(response, tmp, tmplen);        
    if (status != NC_OK) {
        /*
...
redis_post_coalesce_mget(struct msg *request)
{
    ...
    for (i = 0; i < array_n(request->keys); i++) {      /* for each key */
        ...
        //status = redis_copy_rsp_bulk(response, sub_msg, &itemnr);
        status = redis_copy_req_bulk(response, sub_msg, &itemnr);
        if (status != NC_OK) {
            response->owner->err = 1;
            return;
        }
...
redis_post_coalesce(struct msg *r)
{
    ...
    case MSG_REQ_REDIS_BROADCAST:
        return redis_post_coalesce_broadcast(r);
    default:
        NOT_REACHED();
    }
}
...
redis_handle_auth_req(struct msg *req, struct msg *rsp)
{
    ...
    valid = (keylen == pool->client_auth.len) &&
            (memcmp(pool->client_auth.data, key, keylen) == 0) ? true : false;
    if (valid) {
...
redis_add_auth(struct context *ctx, struct conn *c_conn, struct conn *s_conn)
{
    ...
    status = msg_prepend_format(msg, "*2\r\n$4\r\nAUTH\r\n$%d\r\n%s\r\n",
                                pool->password.len, pool->password.data);
    if (status != NC_OK) {
        msg_put(msg);
        return status;
    }
