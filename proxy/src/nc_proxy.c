...
proxy_accept(struct context *ctx, struct conn *p)
{
    ...
    c->sd = sd;
    strcpy(c->peer_ip, nc_unresolve_peer_desc(c->sd));    

    /* set keepalive opt on client socket to detect socket dead 
         if we use bgw which timeout is 90s, to avoid socket leak, we check socket 
         60s and only once */
    status = nc_set_keepalive(c->sd, nci.client_tcp_keepalive_timeout);
    if (status < 0) {
        log_error("set keepalive on s %d for client failed: %s",
                  c->sd, strerror(errno));
        c->close(ctx, c);
        return status;
    }
    ...
    struct server_pool *pool = c->owner;

    /* limitation of max clients, if the value of client_connections equals
     * CONF_DEFAULT_CLIENT_CONNECTIONS, the switch of fd-limitation will be
     * turn off */
    if (pool->client_connections != CONF_DEFAULT_CLIENT_CONNECTIONS) {
        if (pool->client_connections_cnt > pool->client_connections) {
            pool->connection_is_full++;
            
            if (pool->connection_is_full <= 10) {
                log_error("The connection of server_pool:'%.*s' has reached the limitation:%d", 
                          pool->name.len, pool->name.data, pool->client_connections_cnt);
            }

            /* reset this counter when the connection warning 
             * has reached to the MAX_CONNECTION_WARNING in
             * short time so that the warning message of "THE
             * CONNECTION IS FULL" will be printed out */
            if (pool->connection_is_full == MAX_CONNECTION_WARNING) {
    ...
    /* Verify the permission of authorized IP */
    if (ctx->whitelist == 1) {
        if (nc_ip_verify(c->sd,&(c->perm)) == -1) {
            /* The strategy of ip forbiden log is the same as "CONNECTION IS FULL" */
            pool->forbidden_counter++;
            
            if (pool->forbidden_counter <= 10) {
                log_error("ip %s is forbidden.", c->peer_ip); 
            }
            
            if (pool->forbidden_counter == MAX_FORBIDDEN) {
