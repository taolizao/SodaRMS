...
#define SLOT_CONTINUUM_ADDITION   10  /* # extra slots to build into continuum */
#define SLOT_POINTS_PER_SERVER    1
...
slot_update(struct server_pool *pool)
{
    ...
    continuum_addition = SLOT_CONTINUUM_ADDITION;
    points_per_server = SLOT_POINTS_PER_SERVER;
    ...
    if (total_weight > pool->nserver_continuum) {
        ...
        uint32_t nserver_continuum = total_weight + SLOT_CONTINUUM_ADDITION;
        uint32_t ncontinuum = nserver_continuum *  SLOT_POINTS_PER_SERVER;
...
uint32_t
slot_dispatch(struct continuum *continuum, uint32_t ncontinuum, uint32_t hash)
{
    struct continuum *c;
    ...
    c = continuum + hash % SLOT_SIZE % ncontinuum;
