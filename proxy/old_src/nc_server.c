...
server_get_addr_from_pname(struct string *ppname, struct string *ip, int *port)
{
    rstatus_t status;
    uint8_t *p1, *p2, *end, *start, *port_str;

    start = ppname->data;
    end = ppname->data + ppname->len - 1;
    p1 = nc_strchr(start, end, ':');
    p2 = nc_strchr(p1+1, end, ':');
    status = string_copy(ip, start, (uint32_t)(p1-start));
    if(NC_OK != status) {
        return status;
    }
    port_str = nc_strndup(p1+1, (size_t)(p2-p1-1));
    *port = atoi(port_str);
    nc_free(port_str);
    return NC_OK;
}
...
static int
server_address_compare(struct string *pname1, struct string *pname2)
{
    struct string s1_ip, s2_ip;
    int s1_port = 0, s2_port = 0;
    int ret;

    string_init(&s1_ip);
    string_init(&s2_ip);
    server_get_addr_from_pname(pname1, &s1_ip, &s1_port);
    server_get_addr_from_pname(pname2, &s2_ip, &s2_port);

    if(string_compare(&s1_ip, &s2_ip) || s1_port != s2_port) {
        ret = 1;
    } else {
        ret = 0;
    }
...
server_proc_replication_info(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    if (!string_compare(&role_master, &tmp_string)) {
        if (server_address_compare(&server->pname, &server->master->pname)) {
            goto error;
        } else {
            goto success;
        }
...
{
    rstatus_t status;
    struct server_pool *server_pool = NULL;
    struct server *server = NULL, *slave_server;
    struct string pname;
    char pname_buf[NC_PNAME_MAXLEN];
    uint32_t i, pool_n, server_n;
    struct string master_ip;
    int master_port;

    /* find same name server pool & server*/
    pool_n = array_n(&ctx->pool);
    for(i = 0; i < pool_n; i++) {
    ...
    server_n = array_n(&server_pool->server);
    ...
    if (!server_address_compare(&server->pname, &pname)) {
     ...
     */
    string_init(&master_ip);
    master_port = 0;
    server_get_addr_from_pname(&server->pname, &master_ip, &master_port);
    for (i = 0; i < array_n(&server->slave); i++) {
        slave_server = array_get(&server->slave, i);
        if (!server_address_compare(&slave_server->pname, &server->pname)) {
            status = server_set_address(slave_server, server_ip, server_port);
            if (status != NC_OK) {
                string_deinit(&pname);
                string_deinit(&master_ip);
                return status;
            }
        } else if(!server_address_compare(&slave_server->pname, &pname)) {
            status = server_set_address(slave_server, &master_ip, master_port);
            if(status != NC_OK) {
                string_deinit(&pname);
                string_deinit(&master_ip);
                return status;
...
server_rwsplit_pick(struct server *server, msg_type_t type)
{
    ...
    if (type & (PERM_W | PERM_X) || nslave == 0) {
        return server;
    }
...
server_pool_conn(struct context *ctx, struct server_pool *pool, uint8_t *key,
                 uint32_t keylen, msg_type_t type, int idx)
{
    rstatus_t status;
    struct server *server;
    struct conn *conn;
...
rstatus_t
server_add_auth(struct context *ctx, struct conn *c_conn, struct conn *s_conn)
{
    rstatus_t status;
    struct msg *msg;
    struct server_pool *pool;

    ASSERT(!s_conn->client && !s_conn->proxy && !s_conn->sentinel);
