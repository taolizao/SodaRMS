              ...
              struct msg *nmsg)
{
    ...
    /* if server's status is not onlie, we know it's a health check rsp */
    if (server->status != SERVER_STATUS_ONLINE) {
        server_proc_replication_info(ctx, conn, msg);
        return;
    }
...
rsp_send_done(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    /* compute total dealing time, added by taolizao */
    uint64_t msg_dealing_time = 0;
    if(pmsg->narg_start != NULL){
        msg_dealing_time = nc_get_current_timestamp_in_usec() - pmsg->init_time;
        log_slow(pmsg, msg_dealing_time, ctx, NULL);
    }
