...
_conn_get(void)
{
    ...
    conn->recv_bytes = 0;
    conn->pipeline_num = 0;
    conn->pipeline_max_num = 0;
    conn->pipeline_print = 0;
...
conn_authenticated(struct conn *conn)
{
    ...
    ASSERT(!conn->proxy && !conn->sentinel);

    if(conn->client) {
        pool = conn->owner;
        if(!pool->client_need_auth || conn->authenticated) {
            return true;
        }
        return false;
    } else {
        pool = ((struct server *)conn->owner)->owner;
        if(!pool->server_need_auth || conn->authenticated) {
            return true;
        }
        return false;
    }
